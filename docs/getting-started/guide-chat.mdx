...
通过将一个或多个消息传递给聊天模型，可以获取聊天完成。响应也将是一条消息。LangChain当前支持的消息类型为`AIChatMessage`， `HumanMessage`， `SystemMessage` ，和通用`ChatMessage`-- ChatMessage采用任意角色参数，在这里，我们不会使用。大多数时候，您只需要处理`HumanMessage`， `AIChatMessage`和`SystemMessage`。

...
“提示值”是由“提示模板”的“formatPromptValue”方法返回的对象。它可以被转换为一个字符串或一组“ChatMessage”对象。（A `PromptValue` is an object returned by the `formatPromptValue` of a `PromptTemplate`. It can be converted to a string or list of `ChatMessage` objects.）
